...
def kosher_members(c):
    d = c.__dict__
    variables = filter(lambda x: x[0] is not '_' and 
            (isinstance(d[x],int) or 
             isinstance(d[x],str) or
             isinstance(d[x],list) or
             isinstance(d[x],float)), list(d.keys()))
    ...
    while True:
        try:
            if param == 'gain':
                ...
                msg = kromek.Message(type=kromek.MessageType.SET_LLD,
                                     component=kromek.Component.GAMMA_DETECTOR)
                msg.lld = value
                connection.send(msg)
